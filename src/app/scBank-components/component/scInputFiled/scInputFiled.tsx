...
import { Icon } from "@scBank/index";
import { ScBox, ScVFlex, ScText, ScSelectField, ScSelectData } from "@scBank/index";
import { ScInputType, ScInputFieldProps, ScTextFieldProps } from "@scBank/scInputFiled";
...
export const InputFiled = React.forwardRef<HTMLInputElement, ScInputFieldProps>(
  ({ className, placeholder, inputId, disabled, readOnly, type, data, errMsgCheck, ...props }, ref) => {
    ...
    return (
      ...
      >
        {type === "tel" && <ScSelectField data={data ?? []} value={select} onValueChange={setSelect} isTitle={false} />}
  ...
  (
    ...
    return (
      <ScBox>
        ...
        {errMsgCheck && (
          <ScVFlex mt={8}>
            <p id={errorId} className="flex leading-[18px] gap-1 text-sm font-normal sc-text-state-danger">
...
export const ScSearchField = React.forwardRef<HTMLInputElement, ScTextFieldProps>(
  ({ className, labelName, placeholder, fieldType, focusCheck, type = "search", inputId, ...props }, ref) => {
    const finalId = useFinalId(props.id, inputId);
    const resolvedType = resolveInputType(type);
...
ScSearchField.displayName = "ScSearchField";
  ...
  (
    ...
    return (
      <ScBox>
        ...
        {infoMsg && !errMsgCheck && (
          <ScVFlex mt={8}>
            <p id={infoId} className="text-sm font-normal sc-text-basic-04">
...
ScPhoneField.displayName = "ScPhoneField";
